...
<div class="container">
  <div class="row">
   <h2 style="color: red">WARNING!!</h2>
    ...
    <h3>The venue {{$venue}} you are about to update has the following reservations for that particular time;</h3>
   ...
   <tbody>
      ...
      <td>{{ $reservations->Date }}/{{ $reservations->Month}}/{{ $reservations->Year}}</td>
      <td>{{ $reservations->fromTime}}-{{ $reservations->toTime}}</td>
      <td>@if($reservations->rstatus==1)APPROVED
        @elseif($reservations->rstatus==-1)PENDING
      @endif</td>
      <td>{{ $reservations->Reason}}</td>
    ...
    <table style="width: 100%">
      <tr>
        <td style="width:100px">Do you want to notify all about this change and proceed:</td>
        <td style="width:200px">
          <form method="post" action="{{route('notifyAll')}}">
          {{csrf_field()}}
          <input type="text" name="id" value="{{$id}}" hidden>
          <input type="text" name="courseid" value="{{$course}}" hidden>
          <input type="text" name="Reason" value="{{$criteria}}" hidden>
    ...
    <br>
   <div class="col-xs-3">
      <button class="btn btn-dark " onclick="window.location.href='/TimetableManagement';">Back</button>
    </div>
  </div>
  </div>
